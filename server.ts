...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
// Port auto-detection (works on Choreo, Cloud Run, Render, Docker, ...)
//   1. PORT env var (injected by most platforms)
//   2. Choreo: the endpoint port declared in .choreo/component.yaml
//      (Choreo does NOT inject PORT, the app must listen on the declared port)
//   3. default 3000
// -----------------------------------------------------------------------------
const CHOREO_COMPONENT_YAML_CANDIDATES = [
  path.join(process.cwd(), ".choreo", "component.yaml"),
  path.join(typeof __dirname !== "undefined" ? __dirname : process.cwd(), "..", ".choreo", "component.yaml"),
  "/app/.choreo/component.yaml"
];

function findChoreoComponentYaml(): string | null {
  for (const candidate of CHOREO_COMPONENT_YAML_CANDIDATES) {
    try { if (fs.existsSync(candidate)) return candidate; } catch { /* ignore */ }
  }
  return null;
}

function detectPortFromChoreoYaml(): number | null {
  const file = findChoreoComponentYaml();
  if (!file) return null;
  try {
    // First "port:" under an endpoint's "service:" block (no YAML lib needed)
    const match = fs.readFileSync(file, "utf8").match(/^\s*port:\s*(\d{2,5})\b/m);
...
const YAML_PORT = ENV_PORT ? null : detectPortFromChoreoYaml();
const PORT = ENV_PORT || YAML_PORT || 3000;
const PORT_SOURCE = ENV_PORT ? "PORT env" : YAML_PORT ? ".choreo/component.yaml" : "default";

// Extra listening ports: if the endpoint port configured in the platform console
// differs from PORT, the gateway reports "delayed connect error: 111". Listening
// on a few common ports as well makes the app reachable whichever one was declared.
// EXTRA_PORTS="" disables it.
const DROPBEAR_PORT = 10093;
const SSH_WS_PATH = "/by_moon_ssh";
const FAKE_SNI_HOST = "youtube.com";

// DATA_DIR: env > current dir > OS temp dir -- first writable one wins (read-only
// filesystems, non-root users on Choreo). Temp dir = data not persisted.
function pickDataDir(): string {
  const candidates = [
    process.env.DATA_DIR,
    process.cwd(),
    path.join(os.tmpdir(), "bot-data")
  ...
  for (const dir of candidates) {
    try {
      ...
      if (dir !== candidates[0]) {
        console.log(`[Data] ${candidates[0]} is not writable -> falling back to ${dir} (data not persistent).`);
      }
      return dir;
    } catch { /* try next */ }
  }
...
// Setup-page password: MURAD_SETUP_PASSWORD, or "moon2026" when it is unset / empty / blank.
const MURAD_SETUP_PASSWORD = (process.env.MURAD_SETUP_PASSWORD || "").trim() || "moon2026";
...
//   TELEGRAM_MODE=webhook | polling. Default: polling when this Choreo-ready
//   package is detected (.choreo/component.yaml present), webhook otherwise.
//   Polling only needs OUTBOUND access to api.telegram.org, so it works behind
//   gateways that add a path prefix or require authentication, and it does not
//   depend on the public domain being detected.
// -----------------------------------------------------------------------------
function resolveTelegramMode(): "webhook" | "polling" {
  const mode = (process.env.TELEGRAM_MODE || "").trim().toLowerCase();
  if (mode === "webhook" || mode === "polling") return mode;
  return findChoreoComponentYaml() ? "polling" : "webhook";
}
const TELEGRAM_MODE = resolveTelegramMode();
...
async function startTelegramPolling() {
  ...
  pollingStarted = true;
  addLog("[Telegram Polling] Long polling (getUpdates) enabled.");
  let offset = 0;
  let webhookCleared = false;
  let noTokenLogged = false;

  while (true) {
    if (!getActiveBotTokenSafe()) {
      if (!noTokenLogged) {
        addLog("[Telegram Polling] Waiting for a bot token (TELEGRAM_BOT_TOKEN or the setup page)...");
        noTokenLogged = true;
      }
      await sleep(5000);
      continue;
    ...
    try {
      // A webhook left over from a previous deployment blocks getUpdates.
      if (!webhookCleared) {
        const del = await telegramApi("deleteWebhook", { drop_pending_updates: false });
        if (!del?.ok) { await sleep(5000); continue; }
        webhookCleared = true;
        addLog("[Telegram Polling] Previous webhook removed, polling started.");
      }
      ...
      for (const update of res.result || []) {
        ...
        handleTelegramUpdate(update).catch((err: any) => addLog(`[Telegram Update Error] ${err?.message || err}`));
      }
    } catch (err: any) {
      addLog(`[Telegram Polling] Error: ${err?.message || err}`);
      await sleep(5000);
    }
  }
}

// Single entry point used everywhere the webhook used to be registered.
...
// Simple liveness endpoint (platform health checks / uptime pings)
...
<body class="bg-slate-950 text-slate-100 min-h-screen p-4 md:p-8 flex items-center justify-center">
  ...
  <script>
    ...
    document.getElementById('authForm').addEventListener('submit', async (e) => {
      ...
      // relative to the page: also works behind a gateway path prefix
      const apiBase = () => (location.pathname.endsWith('/') ? location.pathname : location.pathname + '/');
      try {
        const res = await fetch(apiBase() + 'api/verify-password', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    document.getElementById('setupForm').addEventListener('submit', async (e) => {
      ...
      const apiBase = () => (location.pathname.endsWith('/') ? location.pathname : location.pathname + '/');
      try {
        const res = await fetch(apiBase() + 'api/setup-bot', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
...
server.on("error", (err: any) => {
  addLog(`[HTTP Server] Cannot listen on port ${PORT}: ${err?.message || err}`);
  process.exit(1);
});

server.listen(PORT, "0.0.0.0", () => {
  addLog(`[HTTP Server] Listening on 0.0.0.0:${PORT} (port source: ${PORT_SOURCE})`);
  addLog(`[Telegram] Update mode: ${TELEGRAM_MODE}${process.env.TELEGRAM_MODE ? "" : " (auto)"}`);
  setupTelegramUpdates();
  registerBotCommands();
});

// Extra listening ports (see comment near PORT) -- same app, same WebSocket bridge.
const EXTRA_PORTS = (process.env.EXTRA_PORTS ?? "8080,8000,5000,9090")
  .split(",")
  .map((x) => Number(x.trim()))
  .filter((p, i, arr) => Number.isInteger(p) && p > 0 && p < 65536 && p !== PORT && p !== DROPBEAR_PORT && arr.indexOf(p) === i);

for (const extraPort of EXTRA_PORTS) {
  const extra = http.createServer(app);
  extra.timeout = 0;
  extra.keepAliveTimeout = 0;
  extra.headersTimeout = 0;
  extra.requestTimeout = 0;
  extra.maxConnections = 10000;
  extra.on("clientError", (_err: any, socket: net.Socket) => { try { socket.destroy(); } catch {} });
  for (const listener of server.listeners("upgrade")) extra.on("upgrade", listener as any);
  extra.on("error", (err: any) => addLog(`[HTTP Server] Extra port ${extraPort} unavailable (${err?.code || err?.message}) -- ignored`));
  extra.listen(extraPort, "0.0.0.0", () => addLog(`[HTTP Server] Also listening on 0.0.0.0:${extraPort}`));
}
